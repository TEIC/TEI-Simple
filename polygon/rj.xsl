<?xml version="1.0" encoding="UTF-8"?><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" xpath-default-namespace="http://www.tei-c.org/ns/1.0" version="2.0"><xsl:template match="hi"><span class="italic"><xsl:apply-templates/></span></xsl:template><xsl:template match="choice"><span class="italic"><xsl:apply-templates select="reg"/></span></xsl:template><xsl:template match="choice"><aaa class="italic"><xsl:apply-templates select="reg"/></aaa></xsl:template><xsl:template match="head[parent::div]"><span class="partHeader"><xsl:apply-templates/></span></xsl:template><xsl:template match="lb[following-sibling::*]"><span class="verse"><xsl:apply-templates/></span><br/></xsl:template><xsl:template match="sp"><div class="sp"><xsl:apply-templates/></div></xsl:template><xsl:template match="speaker"><span class="italic"><xsl:apply-templates/></span></xsl:template><xsl:template match="stage"><div class="stage"><xsl:apply-templates/></div></xsl:template><xsl:template match="g"><span class="stage"><xsl:apply-templates select="@ref"/></span></xsl:template><xsl:template match="ab"><div class="ab"><xsl:apply-templates/></div></xsl:template><xsl:template match="body"><div><xsl:apply-templates/></div></xsl:template><xsl:template match="/"><html xmlns="http://www.w3.org/1999/xhtml"><head xmlns=""><title>
